...
function course(subject, crs, cmp, sctn, days, startTime, endTime, mtgStartDate, mtgEndDate, 
    ...
    this.minStartTime = timeAsMin(startTime)
    this.endTime = endTime;
    this.minEndTime = timeAsMin(endTime)
    ...
    this.cmbndDescr = cmbndDescr;
    this.cmbndEnrlCap = cmbndEnrlCap;
    this.crsTitle = crsTitle;
}
var crsList = [];
allCourses.forEach(function(c) {
    crsList.push(new course(...c))
} )

function timeAsMin(time) {
    var t = time.slice(0, -3)
    var s = t.split(":")
    var min =  parseInt(s[0]) * 60 + parseInt(s[1])
    var p = time.slice(-2)
    if (p === "PM" && parseInt(s[0]) !== 12) {
        min += 720;
    }
    return min
}
